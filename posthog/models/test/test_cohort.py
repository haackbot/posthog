...
class TestCohortDependency(BaseTest):
    ...
    def _assert_depends_on(self, dependent_cohort, referenced_cohort):
        self.assertEqual(list(dependent_cohort.dependencies), [referenced_cohort])
        self.assertEqual(list(referenced_cohort.dependents), [dependent_cohort])

    def _assert_cohorts_have_no_relationships(self, *cohorts):
        for cohort in cohorts:
            self.assertEqual(len(cohort.dependencies), 0, f"Expected no dependencies for {cohort.name}")
            self.assertEqual(len(cohort.dependents), 0, f"Expected no dependents for {cohort.name}")
    ...
    def test_cohort_filter_dependency_created(self):
        ...
        cohort_b = self._create_cohort(
            name="Test Cohort B",
            filters={"properties": {"type": "AND", "values": [{"key": "id", "type": "cohort", "value": cohort_a.id}]}},
        )
    ...
    def test_cohort_filter_dependency_updated_changed(self):
        ...
        cohort_c.filters = {
            "properties": {"type": "AND", "values": [{"key": "id", "type": "cohort", "value": cohort_b.id}]}
        }
        cohort_c.save()
    ...
    def test_cohort_groups_and_filters(self):
        ...
        cohort_b = self._create_cohort(
            ...
            groups=[{"properties": [{"key": "id", "type": "cohort", "value": cohort_a.id}]}],
        )
    ...
    def test_cohort_soft_delete(self):
        ...
        # However if you still have a reference to a deleted cohort, it may still have viable relationships
        self.assertEqual(list(cohort_a.dependents), [cohort_b])
